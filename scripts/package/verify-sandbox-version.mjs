...
console.log('[Sandbox Check] Verifying @cloudflare/sandbox SDK version and checking for updates...');
...
try {
  ...
  try {
    const fallbackPkgPath = path.join(__dirname, '../node_modules/@cloudflare/sandbox/package.json');
...
function findDockerfiles(dir, fileList = []) {
  ...
  for (const file of files) {
    ...
    if (['node_modules', '.git', '.wrangler', 'dist', 'build', '.next', '.svelte-kit'].includes(file)) continue;
...
try {
  latestVersionStr = execSync('pnpm info @cloudflare/sandbox version', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
} catch (error) {
  console.log('[Sandbox Check] Could not check for updates (network issue or pnpm not configured). Proceeding with local validation.');
}
...
if (dockerfiles.length === 0) {
  console.warn('[Sandbox Check] Warning: Could not find any Dockerfiles in the current working directory.');
} else {
  let updatedCount = 0;
      ...
      if (line.startsWith('FROM docker.io/cloudflare/sandbox:')) {
        ...
        if (match) {
          ...
          if (dockerfileVersion !== targetVersion) {
            console.log(`  -> Updating ${path.relative(cwd, filePath)} line ${i + 1}: v${dockerfileVersion} -> v${targetVersion}`);
