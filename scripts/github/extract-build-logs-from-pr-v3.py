...
def provision_cloudflare_bindings(cf_token: str, account_id: str, worker_name: str) -> str:
    ...
    # Using split strings to prevent markdown auto-linkers from mangling the URLs when copied
    url_kv = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/storage/kv/namespaces"
    url_d1 = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/d1/database"
    url_r2 = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/r2/buckets"

    # 1. Provision / Fetch KV
    res_kv = requests.post(url_kv, headers=headers, json={"title": worker_name})
    if res_kv.ok:
        provisioned.append(f"- KV Namespace `{worker_name}` created. ID: `{res_kv.json()['result']['id']}`")
    else:
        res_list = requests.get(url_kv, headers=headers)
        if res_list.ok:
    ...
    res_d1 = requests.post(url_d1, headers=headers, json={"name": worker_name})
    if res_d1.ok:
        provisioned.append(f"- D1 Database `{worker_name}` created. ID: `{res_d1.json()['result']['uuid']}`")
    else:
        res_list = requests.get(url_d1, headers=headers)
        if res_list.ok:
    ...
    res_r2 = requests.post(url_r2, headers=headers, json={"name": worker_name})
...
3. PACKAGE.JSON SCRIPTS:
   ...
   - Ensure the agent implements this exact script pattern:
     ...
     "scripts": {{
        ...
        "build:frontend": "rm -rf src/frontend/.astro && VITE_PUBLIC_API_URL=[https://core-github-api.hacolby.workers.dev](https://core-github-api.hacolby.workers.dev) pnpm --filter frontend build",
...
"""
    ...
    ai_url = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/ai/run/{AI_MODEL}"
    ...
    if response.status_code == 200:
        ...
        if result == {}:
            ...
            choices = result["choices"][0].get("message", {}).get("content", "Could not parse AI response.")
...
def post_pr_comment(comment_body: str, gh_token: str):
    ...
    gh_url = f"https://{'api.github.com'}/repos/{OWNER}/{REPO}/issues/{PR_NUMBER}/comments"
    headers = {
        "Authorization": f"Bearer {gh_token}",
        "Accept": "application/vnd.github.v3+json",
        "Content-Type": "application/json"
    }
    response = requests.post(gh_url, headers=headers, json={"body": comment_body})
...
def main():
    ...
    pr_req_url = f"https://{'api.github.com'}/repos/{OWNER}/{REPO}/pulls/{PR_NUMBER}"
    pr_res = requests.get(pr_req_url, headers=gh_headers)
    ...
    checks_req_url = f"https://{'api.github.com'}/repos/{OWNER}/{REPO}/commits/{head_sha}/check-runs"
    checks_res = requests.get(checks_req_url, headers=gh_headers)
    ...
    account_id, script_name, extracted_uuid = cf_url_match.groups()

    print("\n[4/8] Fetching Build Logs from Cloudflare...")
    cf_headers = {"Authorization": f"Bearer {cf_token}", "Content-Type": "application/json"}

    logs_data = None
    direct_logs_url = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/builds/builds/{extracted_uuid}/logs"
    r_direct = requests.get(direct_logs_url, headers=cf_headers)
    
    if r_direct.status_code == 200:
        logs_data = r_direct.json()
    else:
        list_builds_url = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/builds/workers/{script_name}/builds"
        r_list = requests.get(list_builds_url, headers=cf_headers)
        if r_list.status_code == 200 and r_list.json().get("result"):
            real_build_uuid = r_list.json()["result"][0].get("build_uuid")
            latest_logs_url = f"https://{'api.cloudflare.com'}/client/v4/accounts/{account_id}/builds/builds/{real_build_uuid}/logs"
            r_logs2 = requests.get(latest_logs_url, headers=cf_headers)
