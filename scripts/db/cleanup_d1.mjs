...
async function cleanupDb(binding) {
  ...
  for (const table of tables) {
    ...
    for (const col of TIMESTAMP_COLUMNS) {
      try {
        ...
        if (tsStr) {
          const ageDays =
            (Date.now() - new Date(tsStr).getTime()) / (1000 * 3600 * 24);
  ...
  if (dropStatements.length > 0) {
    console.log(`Executing ${dropStatements.length} drops via bulk file...`);
    const sqlFile = `cleanup_${binding}_temp.sql`;
    writeFileSync(sqlFile, dropStatements.join("\n"));
    
    try {
        execCmd(`npx wrangler d1 execute ${binding} --remote --file="${sqlFile}"`);
        console.log(`Successfully dropped ${dropStatements.length} tables from ${binding}.`);
    } catch(e) {
        console.error("Failed executing drops.");
    } finally {
        unlinkSync(sqlFile);
    }
  } else {
