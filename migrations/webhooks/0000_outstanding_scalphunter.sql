...
CREATE TABLE `planning_requests` (
	...
	`requires_plan_approval` integer DEFAULT true NOT NULL,
	`auto_orchestrate` integer DEFAULT true NOT NULL,
	`auto_implement` integer DEFAULT false NOT NULL,
	...
	`created_at` text DEFAULT CURRENT_TIMESTAMP NOT NULL,
	`updated_at` text DEFAULT CURRENT_TIMESTAMP NOT NULL,
	CONSTRAINT "planning_requests_workstream_check" CHECK("planning_requests"."workstream" in ('api_request', 'project_planning', 'integration_stitch', 'stitch_implementation')),
	CONSTRAINT "planning_requests_status_check" CHECK("planning_requests"."status" in ('queued', 'running', 'awaiting_stitch_approval', 'awaiting_plan_approval', 'approved', 'revising', 'orchestrating', 'implementing', 'completed', 'rejected', 'failed', 'cancelled'))
);
--> statement-breakpoint
CREATE INDEX `planning_requests_workstream_idx` ON `planning_requests` (`workstream`);--> statement-breakpoint
CREATE INDEX `planning_requests_status_idx` ON `planning_requests` (`status`);--> statement-breakpoint
CREATE INDEX `planning_requests_project_idx` ON `planning_requests` (`project_id`);--> statement-breakpoint
CREATE INDEX `planning_requests_session_idx` ON `planning_requests` (`jules_session_id`);--> statement-breakpoint
CREATE INDEX `planning_requests_workflow_idx` ON `planning_requests` (`workflow_instance_id`);--> statement-breakpoint
