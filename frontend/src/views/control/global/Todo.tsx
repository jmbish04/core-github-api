...
import { Plus, Tag, X, StickyNote, CheckCircle2 } from "lucide-react";
...
/** Debounce helper — only fires `fn` after `delay` ms of quiet */
    ...
    <Draggable id={note.id} x={note.x} y={note.y} onMoveEnd={onMoveEnd} zIndex={zIndex} onBringToFront={onBringToFront}>
      ...
      >
        ...
        <button
          ...
          style={{ position: "absolute", top: 6, left: 6, background: "transparent", border: "none", cursor: "pointer", opacity: 0.45, padding: 2 }}
          title="Mark done"
        >
        ...
        <button
          ...
          style={{ position: "absolute", top: 6, right: 6, background: "transparent", border: "none", cursor: "pointer", opacity: 0.45, padding: 2 }}
          title="Remove note"
        >
          <X size={13} />
        </button>
    ...
    <Draggable id={label.id} x={label.x} y={label.y} onMoveEnd={onMoveEnd} zIndex={zIndex} onBringToFront={onBringToFront}>
      <div style={{ transform: `rotate(${label.rotation}deg)`, position: "relative" }}>
        ...
        <button
          ...
          style={{ position: "absolute", top: -4, right: -4, background: "#333", border: "none", borderRadius: "50%", width: 18, height: 18, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", opacity: 0.7, zIndex: 10 }}
          title="Remove label"
        >
          <X size={10} />
        </button>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#1c1c1c", color: "#fff" }}>
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", borderBottom: "1px solid #333", background: "#141414", gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Button variant="ghost" size="sm" onClick={() => setLabelOpen(true)} style={{ color: "#aaa", fontSize: 13 }}>
            <Tag size={14} style={{ marginRight: 6 }} />
            Label
          </Button>
          <Button size="sm" onClick={() => setNoteOpen(true)} style={{ background: "#fde68a", color: "#1a1a1a", fontWeight: 700, fontSize: 13 }}>
      ...
      <div style={{
        ...
        {labels.map(label => (
          <TornLabel key={label.id} label={label} onDelete={handleDeleteLabel} onMoveEnd={handleMoveLabel} zIndex={zMap[label.id] ?? 1} onBringToFront={bringToFront} />
        ))}

        {/* Notes */}
        {notes.map(note => (
          <PostItCard key={note.id} note={note} onDelete={handleDeleteNote} onMoveEnd={handleMoveNote} onMarkDone={handleMarkDone} zIndex={zMap[note.id] ?? 5} onBringToFront={bringToFront} />
        ))}
      </div>
      ...
      <Dialog open={noteOpen} onOpenChange={setNoteOpen}>
        <DialogContent style={{ maxWidth: 440 }}>
          <DialogHeader><DialogTitle>New Post-it Note</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateNote} disabled={!newTitle.trim() && !newContent.trim()} style={{ background: newColor, color: "#1a1a1a", fontWeight: 700 }}>
              Pin to Board
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={labelOpen} onOpenChange={setLabelOpen}>
        <DialogContent style={{ maxWidth: 360 }}>
          <DialogHeader><DialogTitle>New Group Label</DialogTitle></DialogHeader>
          <div style={{ padding: "8px 0" }}>
            <Label style={{ marginBottom: 6, display: "block", fontSize: 12 }}>Label text</Label>
            <Input value={newLabelText} onChange={e => setNewLabelText(e.target.value)} placeholder="e.g. This Week, Bugs, Ideas…" autoFocus onKeyDown={e => { if (e.key === "Enter") handleCreateLabel(); }} />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateLabel} disabled={!newLabelText.trim()}>Add Label</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
