...
export function LiveOpsConsole({ operationId }: LiveOpsConsoleProps) {
  ...
  const [status, setStatus] = useState<
    "idle" | "running" | "completed" | "failed" | "intervention_needed"
  >("idle");
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ Terminal: XTerm }, { FitAddon }, { AttachAddon }] =
        await Promise.all([
          import("@xterm/xterm"),
          ...
          {!operationId ? (
            <Empty className="border-none">
              <EmptyHeader>
                ...
                <EmptyDescription>
                  Start a new workflow from the Command Center or select an
                  active task to view real-time logs.
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          ) : (
