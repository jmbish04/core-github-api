...
export function PRCommandCenter({
        ...
        initialPrs.map(async (pr) => {
          try {
            const res = await fetch(
              `/api/pr/${repoOwner}/${repoName}/${pr.number}/review-status`,
              { credentials: "include" },
            );
            if (res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchOverview = async () => {
      ...
      try {
        const res = await fetch(
          `/api/pr/${repoOwner}/${repoName}/${selectedPrNumber}/overview`,
          { credentials: "include" },
        );
    ...
    const fetchAgent = async () => {
      ...
      try {
        const res = await fetch(
          `/api/pr/${repoOwner}/${repoName}/${selectedPrNumber}/assigned-agent`,
          { credentials: "include" },
        );
        if (res.ok) {
          ...
          <div className="border rounded-lg overflow-x-auto">
            ...
            <ScrollArea className="max-h-[600px]">
              {initialPrs.map((pr) => {
                ...
                const statusInfo =
                  STATUS_LABELS[status?.status || "pending_review"];
                return (
                  <div
                    key={pr.number}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {selectedPr?.author} · Last updated:{" "}
              {selectedPr?.updatedAt
                ? new Date(selectedPr.updatedAt).toLocaleDateString()
                : "—"}
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {selectedPr && (
            <>
              <Badge
                variant="outline"
                className="text-emerald-400 border-emerald-900 bg-emerald-950/20"
              >
                {selectedPr.state}
              </Badge>
              ...
              {assignedAgent && assignedAgent.agent !== "unassigned" && (
                <Badge
                  variant="outline"
                  className={getAgentBadgeClasses(assignedAgent.agent)}
                >
