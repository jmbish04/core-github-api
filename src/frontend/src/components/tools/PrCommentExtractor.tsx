    ...
    const formatForAI = () => {
        ...
        Object.keys(byFile).forEach(file => {
            ...
            byFile[file].forEach(c => {
                text += `#### Line ${c.line || c.original_line || 'Global'}: ${c.user.login === 'gemini-code-assist[bot]' ? 'gemini code reviewer' : c.user.login}\n`;
                if (c.diff_hunk) {
                    text += "```diff\n" + c.diff_hunk + "\n```\n";
                }
                text += `> ${c.body.replace(/\n/g, '\n> ')}\n\n`;
                                            ...
                                            {fileComments.map(comment => (
                                                <div key={comment.id} className="text-sm space-y-2">
                                                    <div className="flex items-center gap-2 text-muted-foreground text-xs">
                                                        <span className="font-medium text-foreground">{comment.user.login === 'gemini-code-assist[bot]' ? 'gemini code reviewer' : comment.user.login}</span>
                                                        <span>•</span>
