...
export function BuildAnalysisCategory() {
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await api.api.hitl[":id"].approve.$post({
        param: { id },
        json: { feedback: feedbackMap[id] || undefined }
      });
      if (res.ok) {
        fetchItems();
      }
  ...
  if (loading) return <div className="text-zinc-500 animate-pulse">Loading CI Healer build analysis queue...</div>;
    ...
    <div className="space-y-6">
      {items.map((item) => {
         ...
         <div key={item.id} className="border border-zinc-800 bg-zinc-950 rounded-lg p-6 flex flex-col space-y-4">
            <div className="flex justify-between items-start">
               <div>
                  <h3 className="text-lg font-medium text-white mb-1">CI Healer: {payload.repoFullName || "Unknown Repo"}</h3>
            ...
            {payload.rawLogs && (
              <div className="space-y-2">
                 <h4 className="text-sm font-semibold text-zinc-300">Build Failure Logs Extract</h4>
                 <div className="bg-zinc-900 border border-zinc-800 rounded p-4 text-xs font-mono text-rose-300 whitespace-pre-wrap max-h-48 overflow-y-auto">
                    {payload.rawLogs}
                 </div>
              </div>
            )}
            ...
            <div className="space-y-2">
               <h4 className="text-sm font-semibold text-zinc-300">Proposed Root Cause & Fix Prompt</h4>
            ...
            {isPending && (
               <div className="space-y-2 pt-2 border-t border-zinc-900">
                  ...
                  <Textarea 
                    placeholder="Enter manual feedback to adjust the CI Healer's repair strategy before dispatching..."
                  ...
                  <div className="flex space-x-3 pt-2">
                    <Button onClick={() => handleApprove(item.id)} variant="default">Approve & Dispatch Repair</Button>
      ...
      {items.length === 0 && (
         <div className="text-center py-10 border border-dashed border-zinc-800 rounded-lg">
            <p className="text-muted-foreground">No pending CI Healer Build Analysis logs to approve.</p>
         </div>
      )}
    </div>
  );
