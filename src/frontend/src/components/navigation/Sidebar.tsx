...
export function Sidebar({ className }: SidebarProps) {
    ...
    const controlCenterLinks = [
        ...
        { name: "Repos", href: "/repos", icon: Github },
    ...
    if (isWorkflowsActive && !isWorkflowsExpanded) setIsWorkflowsExpanded(true);
    if (isWorkshopActive && !isWorkshopExpanded) setIsWorkshopExpanded(true);
    if (isControlCenterActive && !isControlCenterExpanded) setIsControlCenterExpanded(true);
                                    ...
                                    <div className="space-y-1">
                                        {favorites.length === 0 ? (
                                            <p className="px-2 py-1 text-xs text-muted-foreground">
                                                Star repos to keep them pinned.
                                            </p>
                                        ) : (
                                            favorites.slice(0, 8).map((favorite) => {
                                                const href = `/repos/${favorite.repoOwner}/${favorite.repoName}`;
                                                return (
                                                    <NavLink
                                ...
                                <div className="px-3 py-2">
                                    ...
                                    <div className="space-y-1">
                                        {recentProjects.length === 0 ? (
                                            <p className="px-2 py-1 text-xs text-muted-foreground">
                                                Open a repo to see it here.
                                            </p>
                                        ) : (
                                            recentProjects.slice(0, 8).map((project) => {
                                                const href = `/repos/${project.repoOwner}/${project.repoName}`;
                                                return (
                                                    <NavLink
