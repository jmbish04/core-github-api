...
export default function ProjectDashboard() {
  ...
  const basePath = params.owner ? `/repos/${username}/${repoName}` : `/repos/${username}/${repoName}`;
  ...
  const lookupQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/repos/by-repo/${encodeURIComponent(username)}/${encodeURIComponent(repoName)}`,
        { credentials: "include" },
      );
      if (!response.ok) {
  ...
  const overviewQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/repos/${projectId}/overview`, {
        credentials: "include",
      });
  ...
  const detailsQuery = useQuery({
    ...
    enabled: Boolean(projectId),
    queryFn: async () => {
      const response = await fetch(`/api/repos/${projectId}`, { credentials: "include" });
  ...
  const dispatchToJules = async (prompt: string, taskTitle: string) => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/jules/dispatch`, {
        ...
        body: JSON.stringify({
          prompt,
          projectName: overviewQuery.data?.project?.name || "Unknown Project",
          repoFullName: `${username}/${repoName}`
        })
      });
      const data = await res.json() as any;
      if (!res.ok || !data.success) throw new Error(data.error || "Failed to dispatch task");
