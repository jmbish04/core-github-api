...
interface ProjectState {
  ...
  removeFavorite: (repo: Repository) => Promise<void>;
...
export const useProjectStore = create<ProjectState>()((set, get) => ({
  ...
  fetchFavorites: async () => {
    set({ isLoading: true });
    try {
      const resp = await fetch(`/api/projects/favorites`, { credentials: "include" });
      if (resp.ok) {
  ...
  addFavorite: async (repo) => {
    ...
    try {
      const resp = await fetch("/api/projects/favorites", {
        ...
        body: JSON.stringify({ repoOwner: repo.owner, repoName: repo.name }),
        credentials: "include"
      });
      if (resp.ok) {
  ...
  removeFavorite: async (repo) => {
    try {
      const resp = await fetch(`/api/projects/favorites/${encodeURIComponent(repo.owner)}/${encodeURIComponent(repo.name)}`, {
        method: "DELETE",
        credentials: "include"
      });
      if (resp.ok) {
