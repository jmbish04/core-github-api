/**
 * @file automations/pr/ingest/health.ts
 * @description Health check for the PRIngest automation.
...
export async function checkHealth(env: Env): Promise<HealthStepResult> {
  ...
  await runCheck("github_app_auth", async () => {
    ...
    if (!appId) throw new Error("GITHUB_APP_ID missing — PRIngest requires App auth");
  ...
  await runCheck("webhooks_db", async () => {
    if (!env.DB_WEBHOOKS) throw new Error("DB_WEBHOOKS binding missing — PRIngest persists to webhooks DB");
  ...
  return {
    ...
    message: hasFailure ? "PRIngest dependencies degraded" : "PRIngest healthy",
