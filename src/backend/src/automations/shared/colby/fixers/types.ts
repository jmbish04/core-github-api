...
export class WorkerTypesFixer implements Fixer {
  ...
  async execute(ctx: ColbyCommandContext, audit: AuditResult): Promise<boolean> {
    console.log(`[WorkerTypesFixer] Fixing ${audit.filePath}...`);

    try {
      const { data: fileData } = await ctx.octokit.rest.repos.getContent({
  ...
  async fixAll(ctx: ColbyCommandContext): Promise<string> {
    ...
    try {
      ...
      const { data: search } = await ctx.octokit.rest.search.code({ q: query });
      ...
      for (const file of files) {
        if (file.path.endsWith('worker-configuration.d.ts')) {
          continue;
        }
...
export const FixTypesCommand: ColbyCommandDefinition = {
  domain: 'push',
  name: 'fix-types',
