...
export const { Agent, handler } = createAgent<Env>({
  ...
  system: async (ctx: { env: Env }) => {
    const skills = await buildSkillContext(ctx.env as any, 'PlannerAgent');
    return `Create an implementation plan for the user goal. Return a concise, execution-ready plan.${skills}`;
  },
  binding: 'PLANNER',
  tools: [],
