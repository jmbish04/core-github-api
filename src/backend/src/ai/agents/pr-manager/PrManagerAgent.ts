...
import { getAgentDb, migrateAgentDb, agentSchema } from '../../../db/schemas/agents/stateful';
...
export class PrManagerAgent extends runtime.Agent {
  async fetch(request: Request) {
    ...
    if (url.pathname === '/api/jobs') {
      ...
      const results = db.select().from(agentSchema.prManagerJobs).orderBy(desc(agentSchema.prManagerJobs.createdAt)).limit(50).all();
          ...
          try {
            ...
            if (mergeFailed) {
              ...
              if (!allResolved) {
                ...
                db.insert(agentSchema.prManagerJobs).values({ owner, repo, pullNumber: pr.number, status: 'conflict_commented', createdAt: Date.now(), updatedAt: Date.now() }).run();
                await execInSandbox('git merge --abort');
                 ...
                 db.insert(agentSchema.prManagerJobs).values({ owner, repo, pullNumber: pr.number, status: 'conflict_resolved', createdAt: Date.now(), updatedAt: Date.now() }).run();
              }
            } else {
             ...
             db.insert(agentSchema.prManagerJobs).values({ owner, repo, pullNumber: pr.number, status: 'conflict_failed', createdAt: Date.now(), updatedAt: Date.now() }).run();
          } finally {
