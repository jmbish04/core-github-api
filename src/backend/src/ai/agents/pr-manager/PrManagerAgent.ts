...
import { getAgentDb, migrateAgentDb, type AgentDb } from '../../../db/schemas/agents/stateful';
...
export class PrManagerAgent extends runtime.Agent {
  ...
  async fetch(request: Request) {
    ...
    if (url.pathname === '/api/jobs') {
      await this.onStart();
      const results = this.db.select().from(prManagerJobs).orderBy(desc(prManagerJobs.createdAt)).limit(50).all();
    ...
    try {
      ...
      for (const pr of pullsResponse.data) {
        ...
        if (prDetails.data.mergeable_state === 'dirty') {
          ...
          const sessionId = `pr-${pr.number}-${crypto.randomUUID()}`;
          ...
          try {
            ...
            if (mergeFailed) {
              ...
              if (!allResolved) {
                ...
                this.db.insert(prManagerJobs).values({
                  id: crypto.randomUUID(),
                  owner,
                  repo,
                  pullNumber: pr.number.toString(),
                  status: 'conflict_commented',
