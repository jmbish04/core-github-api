...
export class HealthDiagnostician extends HealthDiagnosticianDurableObject {
  ...
  private async handleDiagnose(request: Request) {
    ...
    const skills = await buildSkillContext(this.env as any, 'HealthDiagnostician');
...
Conclude your investigation with a detailed summary containing the severity, rootCause, suggestedFix, and prUrl.${skills}`;
