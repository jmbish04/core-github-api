...
export const githubTools: GitHubToolRegistry = {
  ...
  async attachMCP(env: Env, callbackHost: string): Promise<RunnableTool[]> {
    ...
    const GITHUB_PERSONAL_TOKEN = getSecret(env, "GITHUB_PERSONAL_ACCESS_TOKEN");
    const logger = new Logger(env, "GitHubTools");
    const logPrefix = "[GitHubTools] MCP attachment: ";
    if (!GITHUB_PERSONAL_TOKEN) {
      logger.warn(`${logPrefix} GITHUB_PERSONAL_ACCESS_TOKEN not configured, skipping MCP attachment`);
      return [];
    }
    ...
    try {
      ...
      const targetUrl = "https://api.githubcopilot.com/mcp";
      ...
      logger.info(`${logPrefix} Successfully attached MCP Server with tools: ${mcpToolsList.tools}`);
      ...
      logger.info(`${logPrefix} Successfully attached MCP Server with ${mappedTools.length} tools`);
      return mappedTools;
    } catch (error) {
      logger.error(`${logPrefix} MCP integration failed, falling back to internal tools:`, error);
      return [];
    }
  },
};
...
export async function retryGitHubCall<T>(
  ...
  const logPrefix = "[GitHubTools] retryGitHubCall: ";
  for (let i = 0; i < maxRetries; i++) {
    try {
      console.log(`${logPrefix} Attempt ${i + 1} of ${maxRetries}`);
      return await fn();
    } catch (error: any) {
