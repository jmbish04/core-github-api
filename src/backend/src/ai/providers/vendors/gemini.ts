...
async function executeWithFallback<T>(
  ...
  executionFn: (model: string) => Promise<T>
    ...
    const logger = new Logger(env, "GeminiFallback");
    const logPrefix = "[GeminiFallback - executeWithFallback] ";
  try {
    return await executionFn(originalModel);
  } catch (error: any) {
    logger.warn(`${logPrefix} Initial execution failed for model ${originalModel}:`, { error: error?.message });
    ...
    logger.warn(`${logPrefix} Retrying with alternative model: ${fallbackModelInfo.id}`);
...
export async function getGoogleModels(env: Env, filter?: ModelFilter): Promise<UnifiedModel[]> {
    let data: any;
    const logger = new Logger(env, "Gemini");
    const logPrefix = "[Gemini - getGoogleModels] ";
    try {
        const client = await getGeminiClient(env);
        data = await client.getModels();
        logger.info(`${logPrefix} Fetched Google models: ${JSON.stringify(data)}`);
    } catch (error: any) {
        logger.error(`${logPrefix} Failed to fetch Google models:`, { error: error.message });
        return [];
    }
    ...
    const filteredModels = filter ? models.filter(m => m.capabilities.includes(filter)) : models;
    logger.info(`${logPrefix} Filtered models: ${JSON.stringify(filteredModels)}`);
