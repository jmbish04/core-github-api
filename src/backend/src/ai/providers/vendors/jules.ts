...
export async function generateStructuredResponse<T>(env: Env, prompt: string, schema: z.ZodType<T>, systemPrompt?: string, options?: AIOptions): Promise<T> {
  ...
  const cfg = await ai.getAgentFunctionConfig('Jules', 'structureResponse');
  const step2SystemPrompt = cfg?.systemInstructions || "You are a rigid JSON formatting tool.";
  
  const step2Options: AIOptions = {
    ...options,
    provider: 'worker-ai',
    model: workerAi.STRUCTURING_MODEL,
    skillContext: options?.skillContext
  };

  return await ai.generateStructuredResponse<T>(extractionPrompt, schema, step2SystemPrompt, step2Options);
}
...
export async function generateStructuredWithTools<T>(
  ...
  const ai = new AIProvider(env);
  const cfg = await ai.getAgentFunctionConfig('Jules', 'structureResponse');
  const step2SystemPrompt = cfg?.systemInstructions || "You are a rigid JSON formatting and tool extraction tool.";
  ...
  return await ai.generateStructuredWithTools<T>(extractionPrompt, schema, tools, step2SystemPrompt, step2Options);
}
...
export async function createPlan(env: Env, prompt: string, githubRepoUrl?: string): Promise<string> {
  ...
  let topics: string[] = [];
  try {
    const extracted = await workerAi.generateStructuredResponse<{ topics: string[] }>(env, extractionPrompt, topicExtractionSchema, "You are a senior engineering manager.");
    ...
    for (const topic of topics) {
      ...
      try {
        if (topic.toLowerCase().includes("cloudflare")) {
           const rewriteSchema = z.object({ rewritten_question: z.string() });
           const rewritten = await workerAi.generateStructuredResponse<{ rewritten_question: string }>(env, `Original Question: Provide details on: ${topic}\nTags: cloudflare`, rewriteSchema, "You are a technical documentation assistant. Rewrite the user question to be clear, comprehensive, and optimized for querying Cloudflare documentation.");
           details = rewritten.rewritten_question;
