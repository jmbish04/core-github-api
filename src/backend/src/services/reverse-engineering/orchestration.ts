...
function buildScreenshotTargets(
  ...
  const candidates: FrontendRouteInventoryItem[] = [...summary.frontend.routes]
...
async function collectParallelResearch(
  ...
  const results = await Promise.all(
    sessions.map(async (session: any, index: number) => {
      ...
      const evtStart: Omit<ReverseEngineeringMonitorEvent, 'snapshotId' | 'ts'> = {
        type: 'JULES_PARALLEL_STARTED',
        status: 'running',
        title: spec.title,
        message: `Running: ${spec.title}...`,
        data: { sessionId: session.id || spec.sessionId, key: spec.key },
      };
      await broadcastReverseEngineeringEvent(
        env,
        input.snapshotId,
        evtStart
      );
