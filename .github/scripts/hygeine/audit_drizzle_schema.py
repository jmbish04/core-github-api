...
def main():
    ...
    def append_db_list(md_array, sorted_keys, title):
        md_array.append(f"### {title}")
        if sorted_keys:
    ...
    def append_db_table_report(md_array, sorted_keys, db_map, title):
        md_array.append(f"## {title} d1 db")
        md_array.append("| Table Name | Short File Paths |")
        md_array.append("|---|---|")
        if sorted_keys:
            for t in sorted_keys:
                paths = ", ".join([f"`{p}`" for p in sorted(db_map[t])])
                md_array.append(f"| **{t}** | {paths} |")
        else:
