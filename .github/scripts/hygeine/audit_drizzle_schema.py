...
def main():
    ...
    for file_path in files:
        try:
            ...
            uses_db1 = 'env.DB' in content or 'getDb(' in content or 'this.db' in content
            uses_db2 = 'env.DB_WEBHOOKS' in content or 'getWebhooksDb(' in content
    ...
    for db_name, sorted_map in [("env.DB", db1_sorted), ("env.DB_WEBHOOKS", db2_sorted)]:
        prefix = "" if db_name == "env.DB" else "\n"
    ...
    ignored_slop = {
        'audit_logs', 'chat_tags', 'code_review_comment_enrichments',
        'code_review_comments', 'code_review_runs', 'container_logs',
        'operation_logs', 'organization_settings', 'repo_ai_context',
        'repo_drafts', 'research_files', 'secrets_config', 'automation_runs', 'events'
    }
    ...
    for db_name, sorted_map, db_map in [("env.DB", db1_sorted, db1_map), ("env.DB_WEBHOOKS", db2_sorted, db2_map)]:
