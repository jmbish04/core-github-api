...
Platform Proxy: In a unified backend/frontend monolithic worker with Astro, the `@astrojs/cloudflare` adapter in `astro.config.mjs` MUST configure `platformProxy.configPath` to explicitly point to the root `wrangler.jsonc` file. Failure to do so causes Astro to guess the `compatibility_date` and auto-generate duplicate proxy types in the frontend folder.
