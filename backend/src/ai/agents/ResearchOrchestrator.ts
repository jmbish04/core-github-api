/**
 * @file backend/src/ai/agents/ResearchOrchestrator.ts
...
export class ResearchOrchestrator extends AgentWorkflow<any, ResearchWorkflowParams> {
  ...
  private async executeAgent<T>(
    ...
    const { createRunner } = await import("@/ai/agents/base/agent-ai");

    const agent = new Agent({
      name,
      model,
      instructions: system,
      // @ts-ignore - OpenAI SDK internal Zod type mapping
      outputType: outputSchema,
    });

    // Extract provider from namespaced model (e.g., "workers-ai/@cf/...")
    const provider = model.split('/')[0] || "workers-ai";
    const runner = await createRunner(this.env, provider as any, model);
    
    // We pass prompt as a user message
