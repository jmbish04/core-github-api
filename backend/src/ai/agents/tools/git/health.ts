        ...
        if (!env.SANDBOX) {
            ...
            try {
                ...
                const sandbox = getSandbox(env.SANDBOX, "sandbox-health-probe", {
                    sleepAfter: "5m",
                    containerTimeouts: { portReadyTimeoutMS: 15_000 },
                });

                // 2a. Ping test — verifies container runtime is awake
                const pingStart = Date.now();
                await withTimeout(sandbox.client.utils.ping(), 3000, "Sandbox Ping");
                ...
                try {
                    ...
                    await withTimeout(sandbox.writeFile(testPath, "health-ok"), 3000, "FS Write");
                    const readResult = await withTimeout(sandbox.readFile(testPath), 3000, "FS Read");
                    await withTimeout(sandbox.deleteFile(testPath), 3000, "FS Delete");
                ...
                try {
                    await withTimeout(
                        sandbox.gitCheckout("https://github.com/octocat/Hello-World.git", {
                            targetDir: "/tmp/health-git",
                            depth: 1,
                        }),
                        8000,
                        "Git Clone"
