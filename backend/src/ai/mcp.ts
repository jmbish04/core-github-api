...
try {
    ...
    console.warn("[MCP] Failed to polyfill EventSource:", e);
}
...
export async function executeMcpTool(client: Client, name: string, args: any): Promise<string> {
    try {
        ...
        if (!mcpResult || !mcpResult.content) {
            console.warn(`[MCP] Tool '${name}' returned no content.`);
        ...
        return textContent || "Tool executed successfully but returned no text output.";
    } catch (error: any) {
