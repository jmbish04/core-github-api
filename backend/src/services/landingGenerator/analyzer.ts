...
export class WorkerAnalyzer {
    ...
    static async analyzeWorker(config: {
        ...
        const painPoints = this.identifyPainPoints(purpose, features, { wranglerConfig, packageJson });

        // Extract or infer metrics
        const metrics = this.extractMetrics(components, { wranglerConfig, packageJson });
    ...
    private static identifyPainPoints(
        ...
        features?: string[],
        config?: { wranglerConfig?: WranglerConfig, packageJson?: PackageJson }
    ): Array<{
        title: string;
        description: string;
        solution: string;
        ...
        const dynamicPainPoints: { title: string; description: string; solution: string }[] = [];

        if (config) {
            // 1. Detect Agents & Durable Objects
            const agents = (config.wranglerConfig?.durable_objects?.bindings || [])
                .filter((b: any) => b.class_name.includes('Agent') || b.name.includes('AGENT'));

            if (agents.length > 0) {
                dynamicPainPoints.push({
                    title: 'Complex Agent Coordination',
                    description: 'Managing state and communication between multiple AI agents is notoriously difficult and error-prone.',
                    solution: `${agents.length} Specialized Agents (e.g., ${agents[0].class_name}) working in concert with persistent memory.`
                });
            }

            // 2. Detect Containers
            const containers = config.wranglerConfig?.containers || [];
            if (containers.length > 0) {
                dynamicPainPoints.push({
                    title: 'Serverless Compute Limits',
                    description: 'Standard Workers have CPU and memory limits that prevent running heavy data processing or legacy binaries.',
                    solution: 'Seamless integration with Cloudflare Containers to run any Docker image alongside your edge logic.'
                });
            }

            // 3. Detect MCP (Model Context Protocol)
            const hasMcp = config.packageJson?.dependencies?.['@modelcontextprotocol/sdk'] ||
                config.packageJson?.dependencies?.['@modelcontextprotocol/server-sequential-thinking']; // check known mcp deps

            if (hasMcp) {
                dynamicPainPoints.push({
                    title: 'Fragmented AI Tooling',
                    description: 'Connecting LLMs to internal tools requires custom glue code for every new integration.',
                    solution: 'Full MCP support offering standardized, schema-validated tool interfaces that any AI model can use immediately.'
                });
            }
        }

        if (dynamicPainPoints.length > 0) {
            return dynamicPainPoints;
        }
