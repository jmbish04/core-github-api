...
import { detectWranglerConfig } from '@/routes/api/frontend/projects/utils';
...
async function describeScreenshotWithVision(
  ...
  const response = await env.AI.run('@cf/llava-hf/llava-1.5-7b-hf', {
    image: [...bytes],
    prompt: `Describe the page shown in this screenshot from ${url}. Focus on layout, visible components, perceived functionality, and likely user journey.`,
    max_tokens: 768,
  } as any);
  ...
  return generateStructuredResponse<z.infer<typeof VisionSchema>>(
    env,
    description,
    zodToJsonSchema(VisionSchema as any, 'reverse_engineering_vision') as object,
