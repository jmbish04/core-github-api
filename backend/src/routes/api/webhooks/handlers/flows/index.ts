...
async function logRetrofitOperation(
  ...
  try {
    const httpStatus = status === 'success' ? 200 : status === 'skipped' ? 204 : 500;
    await db.insert(schema.requestLogs).values({
      timestamp: new Date().toISOString(),
      level: status === 'failure' ? 'error' : 'info',
      message: `[retrofit] ${action} ${repoName} -> ${status}`,
      method: 'FLOW',
      path: '/flows/retrofit',
      status: httpStatus,
      latencyMs: 0,
...
async function setRepoSecret(
  ...
  try {
    ...
    // 2. Mock libsodium initialization
    const _sodium: any = {
      ready: Promise.resolve(),
      from_base64: (key: string, variant: number) => new Uint8Array(key.length), // Mock implementation
      from_string: (value: string) => new Uint8Array(value.length), // Mock implementation
      crypto_box_seal: (binsec: Uint8Array, binkey: Uint8Array) => new Uint8Array(binsec.length + binkey.length), // Mock implementation
      to_base64: (bytes: Uint8Array, variant: number) => btoa(String.fromCharCode(...bytes)), // Mock implementation
      base64_variants: { ORIGINAL: 1 }, // Mock variant
    };
    await _sodium.ready;
