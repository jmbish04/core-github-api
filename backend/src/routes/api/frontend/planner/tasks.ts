...
async function executeGithubAction<T>(
    ...
    logOptions?: {
        ...
        eventType: string;
        details?: any;
        extractIssueId?: (result: T) => number | null;
        extractDetails?: (result: T) => any;
    }
): Promise<T | null> {
    ...
    if (logOptions) {
        ...
        if (isSuccess && result) {
            if (logOptions.extractIssueId) {
                const extractedId = logOptions.extractIssueId(result);
                if (extractedId) finalIssueId = extractedId;
            }
            if (logOptions.extractDetails) {
                details = { ...details, ...logOptions.extractDetails(result) };
            }
        }
...
async function getRepoContext(c: Context<{ Bindings: Env }>) {
    ...
    if (!repoRecord) {
        return { ...base, owner, repo, error: 'Repo not found', status: 404 };
    }
...
async function getTaskContext(c: Context<{ Bindings: Env }>) {
    ...
    if (!task) {
        return { ...base, id, error: 'Task not found', status: 404 };
    }
...
tasksApi.get('/repos/:owner/:repo/tasks', async (c) => {
    const ctx = await getRepoContext(c as any);
    if ('error' in ctx) return c.json({ success: false, error: ctx.error }, ctx.status as any);

    const { db, repoRecord } = ctx as { db: ReturnType<typeof getDb>, repoRecord: any };
...
tasksApi.post('/repos/:owner/:repo/tasks', async (c) => {
    ...
    const { repoRecord } = ctx as { repoRecord: any };
    ...
    const issue = await executeGithubAction(
        ...
        async (o, n) => await createGitHubIssue(c.env, o, n, title, description, assignee ? [assignee] : undefined),
        {
            ...
            extractIssueId: (result: any) => result?.number,
            extractDetails: (result: any) => ({ html_url: result?.html_url })
        }
    );
...
tasksApi.patch('/tasks/:id', async (c) => {
    ...
    const ctx = await getTaskContext(c as any);
    if ('error' in ctx) return c.json({ success: false, error: ctx.error }, ctx.status as any);

    const { db, requestId, now, id, task } = ctx as { db: ReturnType<typeof getDb>, requestId: string, now: string, id: string, task: any };
    ...
    const syncField = <K extends keyof typeof updatePayload, G extends string>(
        ...
        githubKey?: G,
        githubValue?: any,
        requireTruthForGithub: boolean = false
    ) => {
        if (value !== undefined && value !== currentValue) {
            updatePayload[localKey] = value;
            if (githubKey) {
                if (!requireTruthForGithub || value) {
                    githubUpdates[githubKey] = githubValue !== undefined ? githubValue : value;
                }
            }
        }
    };
    ...
    syncField(title, task.title, 'title', 'title', undefined, true);
    syncField(description, task.description, 'description', 'body', undefined, true);
    ...
    if (Object.keys(githubUpdates).length > 0) {
        await executeGithubAction(
            ...
            task.repoId,
            task.githubIssueId,
            async (owner, name) => await updateGitHubIssue(c.env, owner, name, task.githubIssueId!, githubUpdates),
...
tasksApi.delete('/tasks/:id', async (c) => {
    ...
    await executeGithubAction(
        ...
        task.repoId,
        task.githubIssueId,
        async (owner, name) => await updateGitHubIssue(c.env, owner, name, task.githubIssueId!, { state: 'closed' }),
