...
async function performGithubAction<T>(
    ...
    if (logOptions) {
        let resolvedDetails = typeof logOptions.details === 'function' ? (result ? logOptions.details(result) : undefined) : logOptions.details;
        ...
        if (typeof logOptions.githubIssueId === 'function') {
            resolvedIssueId = result ? logOptions.githubIssueId(result) : null;
...
function calculateTaskTimestamps(status: TaskStatus | undefined, column: KanbanColumn | undefined, currentStartAt: string | null, currentEndAt: string | null, now: string) {
...
tasksApi.patch('/tasks/:id', async (c) => {
    ...
    // Prepare shared update payload and GitHub updates
    ...
    // Sync to GitHub if linked and there are relevant changes
