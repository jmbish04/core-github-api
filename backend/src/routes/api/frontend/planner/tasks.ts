...
async function performGithubAction<T>(
    ...
    logOptions?: {
        requestId: string;
        taskId: string | null;
        issueNumber?: number | null | ((res: T) => number | null);
        eventType: string;
    ...
    if (logOptions) {
        let detailsVal = logOptions.details;
        if (typeof logOptions.details === 'function') {
            detailsVal = result ? logOptions.details(result) : undefined;
        }
        const details = actionError ? { error: actionError, ...detailsVal } : detailsVal;

        let issueId: number | null = null;
        if (logOptions.issueNumber) {
            issueId = typeof logOptions.issueNumber === 'function' ? (result ? logOptions.issueNumber(result) : null) : logOptions.issueNumber;
        }
...
async function getRepoContext(c: Context<{ Bindings: Env }>) {
    ...
    if (!repoRecord) {
        return { error: 'Repo not found', status: 404 as const, ...base };
    }

    return { owner, repo, repoRecord, ...base };
}
...
async function getTaskContext(c: Context<{ Bindings: Env }>) {
    ...
    if (!task) {
        return { error: 'Task not found', status: 404 as const, ...base };
    }
...
function calculateTaskTimestamps(
    ...
    const updates: { startAt?: string | null; endAt?: string | null } = {};

    if ((status === TaskStatus.IN_PROGRESS || kanbanColumn === KanbanColumn.IN_PROGRESS) && !currentStartAt) {
        updates.startAt = now;
    }
...
tasksApi.post('/repos/:owner/:repo/tasks', async (c) => {
    ...
    await logTaskEvent(db, requestId, null, null, 'api_request_create_task', 'pending', { ...c.req.param(), body });
    ...
    const { owner, repo, repoRecord } = ctx;
    ...
    const issue = await performGithubAction(
        ...
        (owner, name) => createGitHubIssue(c.env, owner, name, title, description, assignee ? [assignee] : undefined),
        {
            requestId,
            taskId: null,
            issueNumber: (res: any) => res?.number || null,
...
tasksApi.patch('/tasks/:id', async (c) => {
    ...
    // Prepare Dual-Update Payloads
    const timestampUpdates = calculateTaskTimestamps(nextStatus, nextColumn, now, task.startAt, task.endAt);
    ...
    // Sync to GitHub if updates exist
    const issueId = task.githubIssueId;
    if (Object.keys(ghUpdates).length > 0) {
        // Only state changed from an open task to an open task? (gh logic redundancy catch)
        if (ghUpdates.state && !updatePayload.title && !updatePayload.description && updatePayload.assignee === undefined) {
             const wasOpen = task.status !== TaskStatus.DONE;
             const isNowOpen = nextStatus !== TaskStatus.DONE;
             if (wasOpen && isNowOpen) {
                 delete ghUpdates.state;
             }
        }
        ...
        if (Object.keys(ghUpdates).length > 0) {
            await performGithubAction(
                ...
                task.repoId,
                issueId,
                (owner, name) => updateGitHubIssue(c.env, owner, name, issueId!, ghUpdates),
                { requestId, taskId: id, issueNumber: issueId || undefined, eventType: 'github_issue_update', details: ghUpdates }
            );
        }
    }
...
tasksApi.post('/tasks/:id/comments', async (c) => {
    ...
    const comment = await performGithubAction(
        ...
        task.repoId,
        issueId,
        (owner, name) => createGitHubComment(c.env, owner, name, issueId!, `**${author || 'User'}**: ${content}`),
        { requestId, taskId: id, issueNumber: issueId || undefined, eventType: 'github_comment_create' }
    );
...
tasksApi.delete('/tasks/:id', async (c) => {
    ...
    await performGithubAction(
        ...
        { requestId, taskId: id, issueNumber: issueId || undefined, eventType: 'github_issue_close' }
    );
