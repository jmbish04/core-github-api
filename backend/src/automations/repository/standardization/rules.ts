...
export class RulesStandardization {
    ...
    static async enforce(
        ...
        try {
            ...
            const paths = (tree.tree || []).map((t: { path?: string | null }) => t.path || "").filter(Boolean);
    ...
    private static async applyRule(
        ...
        if (rule.aiInstructions) {
             try {
                ...
                const customizedOutput = await runTextWithModelFallback(
                    env, 
                    provider, 
                    model, 
                    "Customize the file content based on the instructions. Return ONLY the full customized file content. No markdown fences. Do not output any conversational text.", 
                    prompt
                );
                
                let customized = customizedOutput.trim();
