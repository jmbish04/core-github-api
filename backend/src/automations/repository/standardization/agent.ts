...
export class AgentGenerator {
    static async ensureAgent(env: Env, owner: string, repo: string, octokit?: any) {
        ...
        const stack: string[] = [];
        try {
            await github.rest.repos.getContent({ owner, repo, path: "wrangler.toml" });
            stack.push("Cloudflare Workers");
        } catch(e) {
            console.log(`[AgentGen] No wrangler.toml found for ${owner}/${repo}`, JSON.stringify(e));
        }
        try {
            const { data: pkgParams } = await github.rest.repos.getContent({ owner, repo, path: "package.json" });
