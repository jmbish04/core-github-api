...
function ensureOperationIds(doc: Record<string, any>): Record<string, any> {
  ...
  for (const path of Object.keys(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      if (op && typeof op === "object" && !op.operationId) {
        op.operationId = buildOperationId(method, path);
      }
    }
  }
  return { ...doc, paths };
...
export function buildCompleteOpenAPIDocument(baseDoc: Record<string, any>, baseUrl: string): Record<string, any> {
  ...
  // Ensure all operations have operationId for OpenAPI 3.1 compatibility tooling.
  enhanced = ensureOperationIds(enhanced);
