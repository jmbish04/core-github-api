...
import { TopicOrchestratorAgent } from "../agents/TopicOrchestratorAgent";
...
import { researchBriefs, researchCandidates, researchPlans } from "@/db/schema-topic-research";
  ...
  async run(event: WorkflowEvent<ResearchWorkflowParams>, step: WorkflowStep) {
    ...
    await step.do("save-and-filter-candidates", async () => {
        ...
        for (const item of validCandidates) {
            ...
            // If trending, also save to daily_trends
            if (mode === "trending") {
                await db.insert(dailyTrends).values({
                    id: createId(),
                    date: new Date().toISOString().split('T')[0],
                    category: plan.topic || "General",
                    title: item.title || "Trending Item",
                    url: item.url,
                    description: item.judgement.reasoning,
                    sentInEmail: false,
                    createdAt: new Date()
                });
            }
