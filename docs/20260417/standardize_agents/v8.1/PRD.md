# v8.1 — Product Requirements Document

**Date:** 2026-04-25
**Parent plan:** `docs/20260417/standardize_agents/v5/PLAN.md` (foundational architecture still holds)
**Builds on:** `v7/PRD.md` (specialist-delegation invariants are non-negotiable in v8.1)
**Supersedes:** `docs/20260417/standardize_agents/v8/PRD.md` (kept for diff/reference). v8.1 incorporates the post-`docs/new_agents_sdk/subagents.md` revision (subagents are Agent-level, used as a parallel-work primitive — distinct from `getPeerAgent`) and the Skills SkillProvider-adapter-over-D1 decision (no R2 / Octokit / edigraph / KV migration).
...
5. **Subagents** — `Agent.subAgent(Class, name)` + `abortSubAgent()` + `deleteSubAgent()`. Children are **facets of the parent DO** with **isolated SQLite per child**, discovered automatically via `ctx.exports` (no separate DO bindings; declare the child class as a `new_sqlite_classes` entry in the parent's wrangler migration). Public methods on the child become a **typed RPC stub** automatically — no `@callable` decorator needed. Streaming via `RpcTarget` callback. Limitations: `schedule()`, `cancelSchedule()`, `keepAlive()` are not supported inside sub-agents. **Crucially: `subAgent` is on the base `Agent` class, not Think-only**, so it works on `BaseAgent` subclasses today. Source: `docs/new_agents_sdk/subagents.md` and `docs/new_agents_sdk/think.md` (§Sub-agents — Think layers `chat()`-with-`StreamCallback` on top).

The repo today extends the SDK's `Agent` and `AIChatAgent` directly, with a custom service stack (`Logger`, `StateStore`, `SkillManager`, `HitlQueue`, `CollaborationService`, three-layer health framework) and a custom MCP tool registry (~50 tools). The `agents` package is pinned to `"latest"` — itself a v8.1 risk.

The Think docs include a `R2SkillProvider` example for `Session.withContext("skills", { provider })`. Since the repo already has a D1-backed `SkillManager` (Drizzle, in-memory cache, prefetch / validate / resolveEffective), v8.1 treats Think's `SkillProvider` shape as an **adapter** over that manager rather than a storage migration.

The intended outcome of v8.1 is to **absorb the SDK improvements without disturbing v7 invariants** and without a big-bang base-class migration that would risk frontend message-shape regressions in assistant-ui.
...
- **Modified** `wrangler.jsonc` — adds `BROWSER` (Browser Run), `LOADER` (Worker Loader for codemode + browser execute), `tail_consumers` for the Tail Worker, and `WebQueryWorker` in `migrations.new_sqlite_classes`.
...
| `getPeerAgent<T>(env.X_AGENT)` | base classes | All non-Think dispatch (v7 sanctioned). **Only sanctioned cross-specialist channel — never use `this.subAgent` for this.** |
| `SkillManager.resolveEffective()` | `src/backend/src/ai/providers/agent-support/skills.ts` | Wrapped by the new `SkillProvider` adapter (V8-04 sub-step). D1 schema (`agentSkills` via Drizzle) is **not** modified. |
...
- **Subagent confusion with peer dispatch** — engineers may reach for `this.subAgent` to call another specialist (CloudflareAgent, GithubAgent), violating v7 invariants. Mitigation: new non-negotiable rule in PROMPT.md ("`subAgent` is for parent-owned ephemeral children only; cross-specialist dispatch stays on `getPeerAgent`"); the V8-13 POC names the child class `WebQueryWorker` (not `Researcher`) to make ephemerality obvious; verification grep `rg -n "this\.subAgent\(\s*(CloudflareAgent|GithubAgent|CoordinatorAgent|EngineerAgent)" src/backend/src` must return 0.
- **Subagent SQLite schema drift** — each `WebQueryWorker` instance has its own SQLite. Schema needs to be created idempotently inside the child's first method call (or `onStart`). Mitigation: V8-13 child uses `CREATE TABLE IF NOT EXISTS` patterns from the subagents.md examples; no migration system needed for ephemeral state; adding a child to `migrations.new_sqlite_classes` is one-way — keep the storage shape minimal.
- **Subagent limitations** — `schedule()`, `cancelSchedule()`, `keepAlive()` are not supported in subagents. Mitigation: the V8-13 POC does not use them; if a future subagent needs scheduling, the parent schedules and delegates on fire.
...
- No `CoordinatorAgent` or `CloudflareAgent` migration to Think in v8.1.
...
- No consolidation of `CollaborationService` (WebSocket UI rooms, D1-mirrored chat) with `subAgent` (RPC dispatch with streaming callbacks). They solve different problems.
...
- No replacement of `SkillManager`'s D1 storage backend. The `SkillProvider` adapter is a presentation-layer wrapper; it does **not** introduce R2, Octokit-backed, edigraph, or KV storage for skills in v8.1. Source-of-truth migration to a GitHub-synced model is a v9 candidate.
...
1. **`agents` pinned version (V8-01).** Which semver line ships `Think`, `agents/browser/ai`, `agents/observability`? The audit deliverable (`AGENTS_PACKAGE_AUDIT.md`) resolves this before any code change. **Resolved:** `agents@^0.11.5` per `v8.1/AGENTS_PACKAGE_AUDIT.md`.
...
3. **`BaseThinkAgent` skill assembly (V8-04).** Use `configureSession()` to inject skill context blocks (idiomatic Think) or `getSystemPrompt()` string-concat (closer to current `BaseChatAgent` shape)? **Recommend `configureSession()`** because it survives compaction and matches the Think `SkillProvider` example shape.
...
rg -n "export class WebQueryWorker" src/backend/src \
  | wc -l                                                            # must be 1
rg -n "this\\.subAgent\\(" src/backend/src/ai/agents \
  | grep -E "CloudflareAgent|GithubAgent|CoordinatorAgent|EngineerAgent" | wc -l   # must be 0 (no specialist via subAgent)
rg -n "WebQueryWorker" wrangler.jsonc | wc -l                        # must be ≥ 1 (in new_sqlite_classes)

# 5b. SkillProvider adapter (no storage backend swap)
...
rg -n "withContext\\(.skills." src/backend/src/ai/providers/agent-support/base-think-agent.ts \
  | wc -l                                                            # must be ≥ 1
rg -n "new R2SkillProvider|R2_SKILLS_BUCKET" src/backend/src | wc -l # must be 0
rg -n "SkillManager" src/backend/src/ai/providers/agent-support/skill-provider.ts | wc -l   # must be ≥ 1 (adapter wraps it)
...
# - ResearchAgent.executeParallelWebQueries({ queries: ['…','…'] }) fans out to two
#   WebQueryWorker children with different facet names and aggregates their results
```
...
| Package audit | `docs/20260417/standardize_agents/v8.1/AGENTS_PACKAGE_AUDIT.md` | new (V8-01) |
| Observability module | `src/backend/src/ai/observability/{index.ts,subscribers.ts}` | new (V8-02) |
| Wrangler bindings | `wrangler.jsonc` | modify — add `tail_consumers`, `BROWSER`, `LOADER`, `WebQueryWorker` migration (V8-03, V8-06, V8-13) |
| `BaseThinkAgent` shim | `src/backend/src/ai/providers/agent-support/base-think-agent.ts` | new (V8-04) |
| SkillProvider adapter | `src/backend/src/ai/providers/agent-support/skill-provider.ts` | new (V8-04 sub-step) |
...
| `SkillManager` (wrapped, do not modify) | `src/backend/src/ai/providers/agent-support/skills.ts` | unchanged |
...
V8-01 (pin agents + audit)
   ↓
[V8-02 (observability module),
 V8-04 (BaseThinkAgent shim + SkillProvider adapter),
 V8-06 (browser wrapper),
 V8-09 (codemode wrapper)]                                          # parallel; independent
   ↓
[V8-03 (Tail Worker binding) ← V8-02,
 ...
 V8-13 (subagent POC: ResearchAgent.executeParallelWebQueries via WebQueryWorker)]   # depends on V8-01 only
   ↓
