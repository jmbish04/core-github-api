# Agents SDK V8.1 Package Audit

> Identical to the v8 audit. Reproduced here so `v8.1/` is self-contained.
...
- ✅ `Agent` — base class (also exposes `subAgent`, `abortSubAgent`, `deleteSubAgent` — used by V8-13)
- ✅ `callable` — decorator
...
## Subagent Capability (informational, V8-13)

`Agent.subAgent(Class, name)` is on the **base** `Agent` class (confirmed by `docs/new_agents_sdk/subagents.md`). It does **not** require Think. Children are facets of the parent DO with isolated SQLite per child, discovered via `ctx.exports`. Public methods on the child class become a typed RPC stub automatically — no `@callable` decorator. Limitations: `schedule()`, `cancelSchedule()`, `keepAlive()` are not supported in children.

**v8.1 invariant:** `this.subAgent()` is for parent-owned ephemeral children only. Cross-specialist dispatch stays on `getPeerAgent` (v7).
