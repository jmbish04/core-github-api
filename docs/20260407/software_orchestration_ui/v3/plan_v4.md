# Implementation Plan — Software Orchestration UI v3 (Revised v4)
...
| `src/backend/src/ai/agents/CloudflareDocs.ts` | `GuardrailAgent/methods/cloudflare-docs.ts` (moved from Research → Guardrail) |
...
## Step 1 — Purge Dead `honi` Imports (Blocking Fix)
...
After Step 1a + 1b, `pnpm run check` must be green before any new MMoE code is authored.
...
export const sweFleetSessions = sqliteTable(
...

  {
    ...
    role: text("role", {
      enum: ["solo", "fleet-member", "stitch", "merge"],
    }).notNull(),
...
export async function emitMilestone(
  ...
  // 2. (Option A from Step 10.2) Skip direct D1 write — ChatRoom.post() owns

  //    the D1 mirror via its mirrorToD1() call. If Option B is chosen during

  //    integration, add an INSERT OR IGNORE here keyed on `id`.
...
export async function runStitchBuildLoop(
  agent: EngineerAgent,
  requestId: string,
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    await emitMilestone(
      agent,
      requestId,
      `stitch:loop-${iteration}`,
      "in_progress",
      nextPrompt.slice(0, 80),
    );

    const screen = await stitchTools.generateScreen({
    ...
    const verdict = await getAgentByName(
      agent.env.GUARDRAIL_AGENT,
      agent.id.toString(),
    ).then((g: any) =>
      g.evaluatePayload({ type: "stitch-screen", payload: screen }),
    );
...
export async function fetchCloudflareGoldenPath(
  ...
  const results = await CLOUDFLARE_DOCS_TOOLS.search(agent.env, mcpQuery, {
    limit: 5,
  });
...
export async function evaluatePayload(
  ...
  const touchesCloudflare =
    tags.some((t) =>
      /^(workers|durable-objects|workflows|queues|r2|kv|d1|vectorize|ai|hyperdrive|pages)/.test(
        t,
      ),
    ) ||
    /cloudflare|worker|durable ?object|wrangler|\bDO\b/i.test(
      JSON.stringify(payload),
    );

  const cloudflareGoldenPath = touchesCloudflare
    ? await fetchCloudflareGoldenPath(
        agent,
  ...
  agent.ctx.waitUntil(
    agent.memory.addEpisodic(
      `Reviewed ${type}: ${violations.passes ? "PASS" : "FAIL"}`,

      {
        violations: violations.violations,
        touchedCloudflare: touchesCloudflare,
      },
    ),
  );
    ...
    (async () => {
      ...
      await (room as any).post(
        ...
        violations.passes
          ...
          : `Rejected: ${violations.violations.join("; ")}`,

        {
          type: "guardrail_verdict",
          verdict: {
            status: violations.passes ? "approved" : "rejected",
            violations: violations.violations,
            correctionPrompt: violations.correctionPrompt,
          },
        },
      );
    })(),
...
```jsonc
{
  "durable_objects": {
    "bindings": [
      ...
      {
        "name": "JULES_WEBHOOK_BROADCASTER",
        "class_name": "JulesWebhookBroadcaster",
      },
...
### 10.1 End-to-end emission sequence
...
└────────┬─────────┘
         ...
         │ 1. call emitMilestone()

         ▼

┌────────────────────────────────────────────────┐

│  methods/milestones.ts::emitMilestone()         │

│                                                 │

│  a. INSERT into DO SQLite sweMilestones (Drizzle) ── durable hot path

│  b. ctx.waitUntil → INSERT into D1 chatRoomLogs  ── cross-DO visibility

│  c. ctx.waitUntil → getAgentByName(env.CHAT_ROOM,  ── WS fan-out

│       `request-${requestId}`).post(                  (new @callable() added

│         "EngineerAgent",                              in Step 3h)

│         `${name} → ${status}`,

│         { type: "milestone_update", milestone })

│  d. agent.setState({ ...lastMilestone })          ── Agents SDK state sync
...
│  ChatRoom DO instance `request-${requestId}`   │

│  receives post()                                │

│                                                 │

│  e. this.broadcast(JSON.stringify(msg)) ──── fan-out to every WS client

│  f. this.mirrorToD1(msg)              ──── second D1 write (idempotent

│                                              dedupe by id) — OR remove

│                                              the Engineer's 10.1.b write

│                                              to avoid duplication (see

│                                              "Two-write tradeoff" below)
...
│  Every subscribed WS client receives the msg:  │

│                                                 │

│   • Orchestrator (server-side subscriber)       │

│       → reacts via methods/subscribe-rooms.ts   │

│   • Guardrail (server-side subscriber)          │

│       → auto-evaluates pending_review events    │

│   • Frontend Live Simulator useOrchestratorStatus │

│       → upserts milestone into state → rerender │

└───────────────────────────────────────────────┘

```

### 10.2 Two-write tradeoff (implementer decision)

In the sequence above, step `b` (Engineer writes directly to D1 `chat_room_logs`) and step `f` (ChatRoom's own `mirrorToD1`) both end up writing the same logical record. We pick **one**:

- **Option A (recommended)**: Let `ChatRoom.post()` be the single source of D1 truth. Remove step `b` from `emitMilestone`. Pro: one write, no dedupe logic. Con: if ChatRoom is evicted mid-post, the D1 mirror could miss the milestone (but the DO SQLite row still has it, and the next `tail()` call rebuilds).

- **Option B**: Keep both writes, add an `INSERT OR IGNORE` by `id`. Pro: belt-and-suspenders. Con: two writes per milestone.

Go with **Option A** unless integration testing shows reliability gaps.
...
export function useOrchestratorStatus(requestId: string) {
  ...
  function ingest(msg: Message) {
    ...
    if (type === "milestone_update") {
      ...
      setVerdicts((prev) => ({
        ...prev,
        [msg.metadata.verdict.milestoneId ?? "latest"]: msg.metadata.verdict,
      }));
    } else {
...
export async function checkOrchestratorHealth(
  ...
  /* getAgentByName(env.ORCHESTRATOR_AGENT, "global").ping() */
}

export async function checkEngineerHealth(env: Env): Promise<HealthStepResult> {
  /* ENGINEER_AGENT */
}

export async function checkGuardrailHealth(
  env: Env,
): Promise<HealthStepResult> {
  /* GUARDRAIL_AGENT — also pings env.EDGRAPH */
}

export async function checkResearchHealth(env: Env): Promise<HealthStepResult> {
  /* RESEARCH_AGENT */
}
```
...
3. Keep one-release-cycle aliases for `/api/planning/*`, `/api/agent-planning/*`, `/api/agents/session*` that bridge to the new routes.
...
5. Rewire the frontend hooks listed in Step 10b.3 (`Chat.tsx`, `useAgentRuntime.ts`, `DeepResearchChatPage.tsx`, etc.).
...
| `src/backend/src/ai/agents/GuardrailAgent/{index,types,health}.ts` + `methods/*.ts` | New QA agent (incl. `methods/cloudflare-docs.ts` moved from Research) |

| `src/backend/src/ai/agents/ResearchAgent/{index,types,health}.ts` + `methods/*.ts` | New Librarian agent (no cloudflare-docs.ts) |

| `migrations/core/NNNN_chat_room_logs_rename.sql` | D1 ALTER TABLE |

| `src/frontend/src/hooks/useOrchestratorStatus.ts` | WS hook — subscribes to `request-${requestId}` ChatRoom |

| `src/frontend/src/hooks/useOrchestratorRuntime.ts` | Assistant-UI runtime hook for `ORCHESTRATOR_AGENT` |

| `src/frontend/src/views/repos/OrchestratorStatusView.tsx` | Live Simulator status page |

| `docs/20260407/software_orchestration_ui/v3/honi-replacements.md` | Scratch doc for the HoniClient purge (Step 1b) — ephemeral, deleted before merge |
...
| `src/backend/src/ai/mcp/tools/standards.ts` | Fix `honi` import → `from "ai"` (Step 1a) |

| `src/backend/src/workflows/research/discord.ts` | Remove `from '@/ai/agents/honi'` — use native Cloudflare Workflow API (Step 1a) |

| **24 route/workflow files** | Remove `import { HoniClient } from '@utils/honi-client'` and rewrite call sites (Step 1b) |
...
| `src/backend/src/ai/agents/ChatRoom.ts` | Update import to `chatRoomLogs` **and** add `@callable() post()`, `tail()`, `subscribe()` methods (Step 3h) |
...
| `src/frontend/src/views/control/global/Chat.tsx` | Rewire to `ORCHESTRATOR_AGENT` via `useOrchestratorRuntime` |

| `src/frontend/src/views/control/global/useAgentRuntime.ts` | Add named `useOrchestratorRuntime` variant |

| `src/frontend/src/views/research/DeepResearchChatPage.tsx` | Rewire to `RESEARCH_AGENT` |

| `src/frontend/src/components/docs/SoftwareOrchestrationDoc.tsx` | Update prose (ChatRoom = universal substrate, Guardrail owns Cloudflare docs) |

| `src/frontend/src/components/docs/AgentDocLayout.tsx` | Update nav link |

| `src/frontend/src/routes/RepoRoutes.tsx` | Add `/live-simulator` route (alias `/orchestration` for one release) |
...
6. **HoniClient replacement bucket assignments**: For the 24 files purged in Step 1b, each call site must be classified as (a) agent RPC, (b) service singleton, or (c) dead code. Is it acceptable to author the replacement table as a scratch doc during implementation, or do you want that table finalized _before_ Phase A begins so the diff is predictable?

7. **Two-write tradeoff in Step 10.2**: Go with Option A (ChatRoom owns the D1 write) — confirm. Option B adds redundancy at the cost of doubled writes per milestone.

8. **Room ID convention**: The plan uses `request-{requestId}` as the canonical Live Simulator room key, with sibling conventions `epic-{id}`, `task-{id}`, `sprint-{id}`, `session-{julesId}`, `review-{prNumber}`. Confirm this naming scheme before it gets baked into route paths and `useAgent` calls.
